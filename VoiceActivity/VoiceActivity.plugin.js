...
function buildPlugin([BasePlugin, Library]) {
	...
	Contact the plugin developer to inform them of this error.`;
		...
		getClasses("Masked Avatar Class", ["avatarMasked"]).avatarMasked;
		getClasses("Party Members Classes", [
			"wrapper",
			"partyMembers",
			"partyMemberOverflow"
		]);
		...
		// modules/utils.ts
		const { UserStore: UserStore$2, GuildChannelStore, VoiceStateStore: VoiceStateStore$2, ChannelStore: ChannelStore$2, PermissionStore } = Stores;
		...
		function VoiceProfileSection(props) {
			...
			const section = BdApi.React.createElement("div", {
				...
				className: modules_9dbd3268.partyMembers
			}, BdApi.React.createElement(PartyMembers, {
				members,
				guildId: guild?.id,
				partySize: { totalSize: members.length }
			}))), BdApi.React.createElement("div", {
