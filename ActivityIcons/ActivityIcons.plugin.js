 ...
 * @author Neodymium
 * @version 1.0.0
 * @description Changes the default icons next to statuses. First displays icon for a normal activity, platform, or rich presence, and then an icon for listening to Spotify if applicable. Removes icon for streaming, because you can tell by the purple status anyway, and does not change bot statuses.
...
module.exports = class ActivityIcons {
    start() {
        ...
        BdApi.Patcher.after("ActivityIcons", BdApi.findModule(m => "ActivityStatus" === m.default.displayName), "default", (_, [props], ret) => {
            if (props.activities && ret) {
                ...
                const listening = props.activities.some(activity => activity.type === 2);
